...
| [markdown-guide](#markdown-guide) | README・議事録・WBS・メモ等の種別対応 Markdown 作成ガイド |
| [mcp-server-config](#mcp-server-config) | MCP サーバーの設定・管理 |
| [md-to-html](#md-to-html) | Markdown 議事録・レポートの高品質 HTML 変換 |
...
GitHub Markdown Alerts・Mermaid 図解の共通ルールを提供する汎用 Markdown 作成ガイド。
...
5 フェーズ（設計分析 → HTML 変換 → マルチエージェントレビュー → 統合検証 → 修正）で
Markdown 議事録を参照デザイン準拠の HTML に変換するスキル。
